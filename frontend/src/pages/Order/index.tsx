...
function index() {
  ...
  const handleSubmit = async () => {
    ...
    await axios
      ...
      .catch(() => {
        toast.warning(
          "Error ao enviar pedido! se persistir tente novamente mais tarde!"
        );
      });
  };
  ...
  return (
    <main className="w-100 flex items-center justify-center bg-slate-200 flex-col">
      <StepsHeader />
      <div className="my-5 max-w-7xl px-4 md:px-0 w-full flex justify-start items-start flex-col gap-10">
        <div className="grid grid-cols-layout-mobile md:grid-cols-auto-fill justify-center md:justify-between w-full gap-4 ">
          {products.map((item) => (
